...
// Manipular eventos, passo por parâmetro o nome do campo que desejo manipular e faço uma função para pegar o valor recebido naquele input e também coloco ele em minúsculo para não ter problema na hora de buscar
document.addEventListener('input', function() {
    const searchTerm = searchInput.value.toLowerCase();

    // Quando a busca for vazia, vou trazer a tela de busca original
    if(searchTerm === ''){
        resultPlaylist.classList.add('hidden');
        resultsArtists.classList.remove('hidden');
        return;
